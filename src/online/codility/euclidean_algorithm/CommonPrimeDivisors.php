...
namespace online\codility\euclidean_algorithm;
...
class CommonPrimeDivisors
{
    ...
    public static function solution($A, $B)
    {
        ...
        for ($i = 0; $i < $Z; $i++) {
            if (self::arePrimeDivisorsEqual($A[$i], $B[$i])) {
                $equalPositions += 1;
            }
        }
     ...
     * @return bool
     */
    public static function arePrimeDivisorsEqual(int $N, int $M): bool
    {
        // Greatest common divisor contains all common prime divisors
        $gcd = self::gcd($N, $M);

        // If all prime divisors are not common
        if (self::removeCommonPrimeDivisors($N, $gcd) != 1) {
            return false;
        }

        // If all prime divisors are not common
        if (self::removeCommonPrimeDivisors($M, $gcd) != 1) {
            return false;
        }
     ...
     * @return int
     */
    public static function removeCommonPrimeDivisors(int $a, int $b): int
    {
        while ($a != 1) {
            $gcd = self::gcd($a, $b);
