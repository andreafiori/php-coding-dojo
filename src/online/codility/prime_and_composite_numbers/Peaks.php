...
namespace online\codility\prime_and_composite_numbers;
...
class Peaks
{
    ...
    public static function solution($A)
    {
        ...
        $peaksPositions = self::getPeaksPositions($A);

        $divisors = self::findDivisors($N);
        ...
        foreach ($blockSizes as $blockSize) {
            ...
            for ($i = $blockSize; $i <= $N; $i += $blockSize) {
                ...
                $blockHasPeak = self::doesBlockHavePeak($peaksPositions, $blockStartPosition, $blockEndPosition);
     ...
     * ]
     */
    public static function getPeaksPositions(array $A): array
    {
        $peaks = [];
     ...
     */
    public static function findDivisors(int $N): array
    {
        // Divisors
        $divisors = [];
     ...
     * @return bool
     */
    public static function doesBlockHavePeak(array $peaksPositions, int $blockStartPosition, int $blockEndPosition): bool
    {
        $beg = 0;
